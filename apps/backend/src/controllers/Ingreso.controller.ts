...
export const obtenerIngresos = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT id, usuario_id, descripcion, monto, fecha, categoria FROM ingresos WHERE usuario_id = $1 ORDER BY fecha DESC, id DESC',
      [usuarioId]
    );
...
export const eliminarIngreso = async (req: AuthRequest, res: Response) => {
  try {
    const usuarioId = req.user!.userId;
    const { id } = req.params;
